...
import ir.saltech.puyakhan.ApplicationLoader.Companion.applicationContext
import ir.saltech.puyakhan.R
import ir.saltech.puyakhan.data.model.OtpCode
import ir.saltech.puyakhan.ui.view.activity.keepAliveServiceIntent
import ir.saltech.puyakhan.ui.view.activity.localServerServiceIntent
...
private const val OTP_CODE_BUG_REPORT_URL = "https://pk-bugreport.saltech.ir/"
...
internal fun Context.startKeepAliveService() {
    if (!ApplicationLoader.canRunInBackground) return
    try {
...
internal fun Context.startLocalServerService() {
    if (!ApplicationLoader.canRunLocalServer) return
    try {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            startForegroundService(localServerServiceIntent)
        } else {
            startService(localServerServiceIntent)
        }
    } catch (serviceE: Exception) {
        Log.e("PuyaKhan", "Failed to run foreground service for local server.")
        serviceE.printStackTrace()
    }
}

internal fun grantXiaomiPermissions(context: Context) {
    context.startActivity(XiaomiUtilities.getPermissionManagerIntent())
}
...
fun Context.getWifiIpAddress(): String {
    ...
    if (!capabilities.hasTransport(NetworkCapabilities.TRANSPORT_WIFI)) {
        return "0.0.0.0" // وای فای متصل نیست (شاید دیتای موبایل باشد که در شبکه داخلی کاربرد ندارد)
    }

    val wifiManager =
        applicationContext.getSystemService(Context.WIFI_SERVICE) as WifiManager
    val ipAddress = wifiManager.connectionInfo.ipAddress
    return if (ipAddress == 0) "0.0.0.0" else Formatter.formatIpAddress(ipAddress)
}
